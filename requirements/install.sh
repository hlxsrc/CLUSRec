...
# Create environment
if [[ $(workon clusrec > /dev/null 2>&1) ]]; then
    echo "[INFO] Virtual environment CLUSREC is already created, skipping to next step..."
else
    echo "[INFO] Attempting to create CLUSREC virtual enviroment..."
    mkvirtualenv clusrec -p python3 > /dev/null 2>&1
    if [[ $? != 0 ]]; then
...
$HOME/.local/bin/.virtualenvs/clusrec/bin/pip -qq install -r pip.txt
if [[ $? != 0 ]]; then
    echo "[ERR] Installation failed, try installing manually."
else
    echo ""
    ...
    echo ""
    echo "The environment CLUSREC was successfully created"
    echo "In order to run the python scripts activate the CLUSREC virtual environment"
    echo "To do so execute the following command -> # workon clusrec"
fi
